...
export default function TowConfigTable({
  ...
  // Group tows by type
  const groupedTows = useMemo(() => {
    const groups = {};
    tows.forEach((tow, originalIndex) => {
      const type = tow.type || 'task';
      if (!groups[type]) groups[type] = { tows: [], totalWeight: 0 };
      groups[type].tows.push({ ...tow, originalIndex });
      groups[type].totalWeight += (parseFloat(tow.weight_pct) || 0);
    });
    return groups;
  }, [tows]);
  ...
  const colCount = 8 + (hasAdjustments ? 2 : 0);
        ...
        <table className="w-full text-sm border-separate border-spacing-y-3 px-4">
          ...
          <tbody className="">
            {tows.length === 0 && !showAddRow ? (
              ...
              towTypes.filter(tt => groupedTows[tt.value]).map((typeInfo) => {
                const group = groupedTows[typeInfo.value];
                const isCollapsed = collapsedTypes.has(typeInfo.value);
                  ...
                  <Fragment key={typeInfo.value}>
                    ...
                    <tr className="bg-slate-50/50">
                      <td colSpan={colCount} className="px-4 py-2">
                        <div className="flex items-center justify-between">
                          <button 
                            onClick={() => toggleTypeCollapse(typeInfo.value)}
                          ...
                          >
                            <div className={`p-0.5 rounded transition-transform ${isCollapsed ? '-rotate-90' : ''}`}>
                              <ChevronDown className="w-4 h-4" />
                            </div>
                            <span className={`px-2 py-0.5 rounded-md text-[10px] ${getTypeStyle(typeInfo.value)}`}>
                              {typeInfo.label}
                            </span>
                            <span className="opacity-50 font-normal">({group.tows.length})</span>
                          </button>
                          <div className="text-[10px] font-bold text-slate-400">
                            TOTALE PESO: <span className="text-slate-600">{group.totalWeight.toFixed(1)}%</span>
                          </div>
                        </div>
                      </td>
                    </tr>

                    {!isCollapsed && group.tows.map((tow) => {
                      const idx = tow.originalIndex;
                      return (
                        <Fragment key={idx}>
                          <tr className="group bg-white/70 backdrop-blur-md rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-md hover:bg-white/90 hover:-translate-y-0.5 transition-all border border-white/50">
                            <td className="px-4 py-2 rounded-l-xl">
                              <input
                                type="text"
                                value={tow.tow_id}
                                onChange={(e) => handleUpdateTow(idx, 'tow_id', e.target.value.toUpperCase())}
                                disabled={disabled}
                            ...
                            <td className="px-4 py-2 text-center">
                               <span className={`px-2 py-0.5 rounded-md text-[10px] font-medium ${getTypeStyle(tow.type)}`}>
                                 {towTypes.find(tt => tt.value === tow.type)?.label || tow.type}
                               </span>
                            </td>
                            <td className="px-4 py-2">
                              <input
                          ...
                          {tow.type === 'catalogo' && expandedCatalog.has(idx) && (() => {
                            ...
                            return (
                              <tr key={`cat-${idx}`} className="bg-rose-50">
                                <td colSpan={catColCount} className="px-6 py-3">
                                  {clusters.length === 0 ? (
